...
import { DELIVERY_TYPE } from '$lib/enums.js';
import { error } from '@sveltejs/kit';

export let load = async ({fetch, params, locals}) => {

    let order = (await db.query("select * from `order` where user_id = ? and id = ? limit 1;", [locals.user.id, params.order_id]))[0][0];
    if (order.length == 0) {
